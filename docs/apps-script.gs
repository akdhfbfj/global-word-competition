  ...
  try {
    ...
    if (data.type === 'individual') {
      ...
      if (sheet.getLastRow() === 0) {
        sheet.appendRow(['접수번호', '제출일시', '이름', '생년월일', '학교', '학년', '학생연락처', '학부모연락처', '이메일', '부문', '주소', '쿠폰']);
      }
      sheet.appendRow([
        data.receiptNo, data.submittedAt, data.name, data.birthDate,
        data.school, data.grade, data.phoneStudent, data.phoneParent,
        data.email, data.division, data.address, data.coupon
      ]);
    }
    ...
    if (data.type === 'group') {
      ...
      if (sheet.getLastRow() === 0) {
        sheet.appendRow(['접수번호', '제출일시', '단체명', '학원연락처', '학부모연락처', '주소', '인원', '총참가비', '쿠폰']);
      }
      sheet.appendRow([
        data.receiptNo, data.submittedAt, data.orgName, data.phoneAcademy,
        data.phoneParent, data.address, data.participantCount, data.totalFee, data.coupon
      ]);

      const pSheet = ss.getSheetByName('word_participants') || ss.insertSheet('word_participants');
      if (pSheet.getLastRow() === 0) {
        pSheet.appendRow(['접수번호', '이름', '학교', '학년', '부문']);
      }
      (data.participants || []).forEach(function (p) {
        pSheet.appendRow([data.receiptNo, p.name, p.school, p.grade, p.division]);
      });
    }
